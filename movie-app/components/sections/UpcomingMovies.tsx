...
const TrendingMovies = () => {
  ...
  return (
    <section className="px-4 py-6">
      <h2 className="text-3xl font-semibold mb-4 font-mono ">Upcoming Movies</h2>
      ...
      {loading ? (
        ...
        <div className="relative">
          <button
            className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-black p-2 rounded-full shadow-md"
            onClick={() => scroll("left")}
          >
            <ChevronLeft size={24} />
          </button>
          ...
          <div
            ref={scrollRef}
            className="text-sm flex space-x-6 overflow-x-auto scrollbar-hide py-4 px-1"
          >
            {movies.map((movie, index) => (
              <div
                key={movie.id}
                ref={index === 0 ? cardRef : null}
                className="min-w-[150px] max-w-[150px] truncate text-pretty"
              >
                <MovieCard
                  id={movie.id}
                  title={movie.title}
          ...
          <button
            className="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-black p-2 rounded-full shadow-md"
